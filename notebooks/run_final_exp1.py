...
def run_challenge_model(model, data, recordings, verbose):
    ...
    if model['model1'] == 'resmax1_dr' :
        model1 = get_ResMax_o_1_dr(model['mel_shape'],model['cqt_shape'],model['stft_shape'], use_mel = model['use_mel'], use_cqt = model['use_cqt'], use_stft = model['use_stft'], ord1 = model['ord1'])
    if model['model2'] == 'resmax2_dr' :
        model2 = get_ResMax_2_dr(model['mel_shape'],model['cqt_shape'],model['stft_shape'], use_mel = model['use_mel'], use_cqt = model['use_cqt'], use_stft = model['use_stft'])
...
#    use_stft = params_feature['use_stft']
    ...
    model1 = get_ResMax_o_1_dr(mel_input_shape, cqt_input_shape, stft_input_shape, use_mel = use_mel, use_cqt = use_cqt, use_stft = use_stft, ord1 = ord1, dp = dp, fc = fc, ext = ext)
    model2 = get_ResMax_2_dr(mel_input_shape, cqt_input_shape, stft_input_shape, use_mel = use_mel, use_cqt = use_cqt, use_stft = use_stft, dp = dp, fc = fc, ext = ext)
...
#              'ranfilter2' : [3, [18,19,20,21,22,23]]
    ...
    save_challenge_model(model_folder, model1, model2, m_name1 = 'resmax1_dr', m_name2 = 'resmax2_dr', param_feature = params_feature)
