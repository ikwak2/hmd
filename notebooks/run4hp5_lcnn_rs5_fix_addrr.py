...
def run_challenge_model(model, data, recordings, verbose):
    ...
    if model['model1'] == 'lcnn1_dr_rr' :
        model1 = get_LCNN_o_1_dr_rr(model['mel_shape'],model['cqt_shape'],model['stft_shape'], use_mel = model['use_mel'], use_cqt = model['use_cqt'], use_stft = model['use_stft'], ord1 = model['ord1'], dp = model['dp'], fc = model['fc'], ext = model['ext'], ext2 = model['ext2'])
    if model['model2'] == 'lcnn2_dr_rr' :
        model2 = get_LCNN_2_dr_rr(model['mel_shape'],model['cqt_shape'],model['stft_shape'], use_mel = model['use_mel'], use_cqt = model['use_cqt'], use_stft = model['use_stft'], dp = model['dp'], fc = model['fc'], ext = model['ext'], ext2 = model['ext2'])
...
#    nmel = int(nmel)
    ...
    nep = np.random.choice([20,50,80,100])
...
#    if ord1 :
#        features_trn, mm_lbs_trn, out_lbs_trn, mel_input_shape, cqt_input_shape, stft_input_shape = get_features_3lb_all_ord_rr(train_folder, patient_files_trn, **params_feature)
#        features_test, mm_lbs_test, out_lbs_test, _, _, _ = get_features_3lb_all_ord_rr(test_folder, patient_files_test, **params_feature)
#    else :
