...
for i in range(1000):
    ...
    if ord1 :
        features_trn, mel_input_shape, cqt_input_shape, stft_input_shape,interval_input_shape = get_features_3lb_all_ord_rr(train_folder, patient_files_trn, **params_feature)
    else :
...
#     with open('/home/jk21/Downloads/Data/features_trn_final_0827.pickle', 'rb') as f:
#         data1 = pickle.load(f)

#     with open('/home/jk21/Downloads/Data/features_test_final_0827.pickle', 'rb') as f:
#         data2 = pickle.load(f)
...
#                                        features_test['stft1'],features_test['interval_mean']],features_test['out_labels']), 
    ...
    ##########################################################################    
        ...
        if model['model1'] == 'lcnn1' :
            model1 = LCNN_seq_mean_model_1(model['mel_shape'],model['cqt_shape'],model['stft_shape'], model['interval_shape'],
                                           use_mel = model['use_mel'],use_cqt = model['use_cqt'], use_stft = model['use_stft'],
                                           ord1 = model['ord1'], dp = model['dp'], fc = model['fc'], ext = False, ext2= True)  
        if model['model2'] == 'lcnn2' :
            model2 = LCNN_seq_mean_model_2(model['mel_shape'],model['cqt_shape'],model['stft_shape'], model['interval_shape'],
                                           use_mel = model['use_mel'],use_cqt = model['use_cqt'], use_stft = model['use_stft'],
                                           dp = model['dp'], fc = model['fc'], ext = True, ext2=False) 

        
        if model['model1'] == 'lcnn3' :
            model1 = LCNN_mean_model_1(model['mel_shape'],model['cqt_shape'],model['stft_shape'], model['interval_shape'],
                                       use_mel = model['use_mel'],use_cqt = model['use_cqt'], use_stft = model['use_stft'],
                                       ord1 = model['ord1'], dp = model['dp'], fc = model['fc'],ext = False, ext2= True) 
            
        if model['model2'] == 'lcnn4' :
            model2 = LCNN_mean_model_2(model['mel_shape'],model['cqt_shape'],model['stft_shape'], model['interval_shape'],
                                       use_mel = model['use_mel'],use_cqt = model['use_cqt'],use_stft = model['use_stft'],
                                       dp = model['dp'], fc = model['fc'],ext = True, ext2=False)
        ...
        if model['model1'] == 'lcnn5' :
            model1 = LCNN_seq_mean_jk_model_1(model['mel_shape'],model['cqt_shape'],model['stft_shape'], model['interval_shape'],
                                    use_mel = model['use_mel'],use_cqt = model['use_cqt'], use_stft = model['use_stft'],
                                    ord1 = model['ord1'], dp = model['dp'], fc = model['fc'], ext = False, ext2= True)  
        
        if model['model2'] == 'lcnn6' :
            model2 = LCNN_seq_mean_jk_model_2(model['mel_shape'],model['cqt_shape'],model['stft_shape'], model['interval_shape'],
                                           use_mel = model['use_mel'],use_cqt = model['use_cqt'], use_stft = model['use_stft'],
                                           dp = model['dp'], fc = model['fc'], ext = True, ext2=False) 

        
        if model['model1'] == 'lcnn7' :
            model1 = LCNN_mean_jk_model_1(model['mel_shape'],model['cqt_shape'],model['stft_shape'], model['interval_shape'],
                                       use_mel = model['use_mel'],use_cqt = model['use_cqt'], use_stft = model['use_stft'],
                                       ord1 = model['ord1'], dp = model['dp'], fc = model['fc'],ext = False, ext2= True) 
            
        if model['model2'] == 'lcnn8' :
            model2 = LCNN_mean_jk_model_2(model['mel_shape'],model['cqt_shape'],model['stft_shape'], model['interval_shape'],
                                       use_mel = model['use_mel'],use_cqt = model['use_cqt'],use_stft = model['use_stft'],
                                       dp = model['dp'], fc = model['fc'],ext = True, ext2=False)
...
#                     print(tmp_final)
    ...
    fnm = 'result_interval_add_jk_model/result'+ str(tnow)+'.pk'
