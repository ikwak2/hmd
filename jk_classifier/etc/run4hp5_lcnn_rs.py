...
for i in range(50000) :
    model_folder = 'lcnn_dr_'+ str(i)
    output_folder = '/home/ubuntu/hmd/notebooks/tmp/out_lcnn_dr_'+ str(i)

    winlen = np.random.choice([512,256,128,800])
    hoplen = winlen / 2
    winlen = int(winlen)
    hoplen = int(hoplen)
    ...
    trim = np.random.choice([1,4000])
    use_cqt = np.random.choice([True,False])
    ...
    mm_weight = np.random.choice([2,3,4,5])
    oo_weight = np.random.choice([2,3,4,5])
    ord1 = np.random.choice([True,False])
    mm_mean = np.random.choice([True,False])
    dp = np.random.choice([.3,.4,.5,.6])
    fc = np.random.choice([True,False])
    ext = np.random.choice([True,False])
