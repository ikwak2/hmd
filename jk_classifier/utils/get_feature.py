...
def get_features_3lb_all_rr_raw(data_folder, patient_files_trn, 
                          samp_sec=20, pre_emphasis = 0, hop_length=256, win_length = 512, n_mels = 100,
                          filter_scale = 1, n_bins = 80, fmin = 10, trim = 4000,
                         use_mel = True, use_cqt = False, use_stft= False, use_raw = False,use_rr = False,use_wavelet_mel=False
                         ) :
    features = dict()
    features['id'] = []
    features['age'] = []
    ...
    for i in tqdm.tqdm(range(num_patient_files)):
        ...
        for j in range(num_locations) :
            ...
            if use_wavelet_mel:
                ...
                wav_wavelet,__=pywt.cwt(wav,13,'gaus2')
                mel1 = feature_extract_melspec(wav_wavelet[0], samp_sec=samp_sec, pre_emphasis = pre_emphasis, hop_length=hop_length, win_length = win_length, n_mels = n_mels, trim = trim)[0]
            else:
                mel1 = np.zeros((1,1))
            ...
            if use_cqt :
                mel2 = feature_extract_cqt(filename, samp_sec=samp_sec, pre_emphasis = pre_emphasis, 
                                           filter_scale = filter_scale, n_bins = n_bins, fmin = fmin, trim = trim)[0]
            else :
                mel2 = np.zeros( (1,1) )                
            features['cqt1'].append(mel2)
...
#                    current_out_labels = np.array([1, 0])
    ...
    return features, mel_input_shape, cqt_input_shape, stft_input_shape,wavelet_mel_input_shape
